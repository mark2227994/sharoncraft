...
export default async function handler(req, res) {
  if (req.method === "GET") {
    const data = await readAdminContentField("articles", defaultArticles);
    return res.status(200).json(Array.isArray(data) ? data : defaultArticles);
  }
  ...
  if (req.method === "POST") {
    ...
    try {
      const payload = Array.isArray(req.body) ? req.body : defaultArticles;
      await writeAdminContentField("articles", payload);
