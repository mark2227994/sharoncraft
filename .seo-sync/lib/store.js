...
import { normalizeCategoryName, normalizeSubcategoryName } from "./categories";
...
let lastSyncedProductCatalogSignature = "";
...
function mapTableProductToCatalogProduct(product) {
  ...
  return {
    ...
    fulfillmentType:
      normalizeText(product?.fulfillment_type || product?.fulfillmentType) ||
      "ready_to_ship",
    careInstructions,
    details: [
...
function sortShopProducts(products, sortBy) {
  if (sortBy === "newest") {
...
function applyBroadTableFilters(query, {
  activeCategory = "all",
  activeSubcategory = "",
  availability = "all",
  ...
  if (activeCategory !== "all") {
    nextQuery = nextQuery.eq("category", activeCategory);
  }

  if (activeSubcategory) {
    nextQuery = nextQuery.eq("subcategory", activeSubcategory);
  }

  if (availability === "in_stock") {
    nextQuery = nextQuery.gt("stock_quantity", 0);
  }

  if (activePriceRange === "under-1000") {
    nextQuery = nextQuery.lte("price", 999);
  } else if (activePriceRange === "1000-3000") {
    nextQuery = nextQuery.gte("price", 1000).lte("price", 3000);
  } else if (activePriceRange === "3000-5000") {
    nextQuery = nextQuery.gte("price", 3001).lte("price", 5000);
  } else if (activePriceRange === "above-5000") {
    nextQuery = nextQuery.gte("price", 5001);
  }
...
export async function queryShopProducts({
  ...
  const normalizedCategory =
    activeCategory === "all" ? "all" : normalizeCategoryName(activeCategory) || "all";
  const normalizedSubcategory =
    normalizedCategory !== "all"
      ? normalizeSubcategoryName(normalizedCategory, activeSubcategory)
      : "";

  let products = [];
  let usedSupabaseResult = false;
  ...
  try {
    ...
    const { data, error } = await applyBroadTableFilters(baseQuery, {
      activeCategory: normalizedCategory,
      activeSubcategory: normalizedSubcategory,
      availability,
      activePriceRange,
      sortBy,
    });
    ...
    if (!error && Array.isArray(data)) {
      usedSupabaseResult = true;
      products = normalizeProducts(data.map(mapTableProductToCatalogProduct));
    }
  } catch {
    // Fall through to the existing resilient source chain below.
  }

  if (!usedSupabaseResult) {
    products = await readProducts();
  }

  const filteredProducts = sortShopProducts(
    products
      .filter((product) => {
        if (normalizedCategory === "all") return true;
        return normalizeText(product?.category) === normalizedCategory;
      })
      .filter((product) => {
        if (!normalizedSubcategory) return true;
        return normalizeText(product?.subcategory) === normalizedSubcategory;
      })
      .filter((product) =>
        availability === "in_stock"
          ? !product.isSold && Number(product.stock || 0) > 0
          : true,
      )
      .filter((product) => matchesPriceRange(product, activePriceRange))
      .filter((product) => {
        if (activeProductType === "all") return true;
        return product?.fulfillmentType === activeProductType;
      }),
    sortBy,
  );
...
async function syncProductFallbackCatalog(products) {
  const normalizedProducts = normalizeProducts(products);
  const signature = JSON.stringify(normalizedProducts);

  if (signature === lastSyncedProductCatalogSignature) {
    return;
  }

  lastSyncedProductCatalogSignature = signature;
  ...
  try {
    ...
    // Ignore site_settings sync failures and keep serving the live table.
  }

  try {
    await writeJsonFile(FILES.products, normalizedProducts);
  } catch {
    // Ignore local fallback sync failures in read-only environments.
  }
}
...
export async function readProducts() {
  try {
    ...
    if (!error && Array.isArray(data)) {
      if (data.length > 0) {
        const normalizedProducts = normalizeProducts(data.map(mapTableProductToCatalogProduct));
        void syncProductFallbackCatalog(normalizedProducts);
...
export async function writeProducts(products) {
  const normalizedProducts = normalizeProducts(products);
  await syncProductFallbackCatalog(normalizedProducts);
}
