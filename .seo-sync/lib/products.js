...
const CATEGORY_MAP = new Map([
  ...
  ["bridal-occasion", "African Wear"],
  ["occasion", "African Wear"],
  ["african wear", "African Wear"],
  ["african-wear", "African Wear"],
  ["gift set", "Gifted Carry"],
  ...
  ["home-decor", "Home & Living"],
  ["home object", "Home & Living"],
  ["home objects", "Home & Living"],
  ["home and living", "Home & Living"],
  ["home living", "Home & Living"],
  ["woodcarvings", "Home & Living"],
  ["soapstone", "Home & Living"],
  ["ceramics", "Home & Living"],
...
function inferJewelryType(product, rawCategory, safeName) {
  // Priority: Use explicit jewelry_type field first (from Supabase)
  if (product?.jewelry_type) {
    const normalized = normalizeJewelryType(product.jewelry_type);
    if (normalized) return normalized;
  }

  // Fallback: Infer from other sources
  const candidates = [
    product?.jewelryType,
    rawCategory,
    safeName,
...
export function normalizeProduct(product) {
  ...
  return {
    ...product,
    id: compactText(product?.id) || slug || safeName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, ''),
...
export function getRelatedProducts(products, currentSlug, limit = 4) {
  const normalized = normalizeProducts(products);
  const current = normalized.find((p) => p.slug === currentSlug);
  if (!current) return [];
