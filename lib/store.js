 ...
 *
 * Products prefer the Supabase `products` table (admin-v2 / App Router admin) so
 * storefront and admin share one catalog. Results are mirrored to
 * site_settings.products_catalog for backward compatibility. Falls back to the
 * legacy catalog blob, then data/store/products.json.
 *
...
import { mapTableRowToCatalogProduct } from "./catalog-sync";
...
async function getLocalStoreDependencies() {
  const runtimeRequire = typeof require === "function" ? require : eval("require");
  const path = runtimeRequire(["pa", "th"].join(""));
  const fs = runtimeRequire(["fs", "promises"].join("/"));
...
async function readJsonFile(filePath) {
  const { fs } = await getLocalStoreDependencies();
  const raw = await fs.readFile(filePath, "utf8");
  return JSON.parse(raw);
}

async function syncProductCatalogCache(products) {
  ...
  try {
    ...
    await supabase.from("site_settings").upsert(
      {
        key: CATALOG_KEY,
        value: normalizedProducts,
        updated_at: new Date().toISOString(),
      },
      { onConflict: "key" },
    );
  } catch {
    // Ignore cache sync failures; live table read still works.
  }

  try {
    const { fs, files } = await getLocalStoreDependencies();
    await fs.writeFile(files.products, `${JSON.stringify(normalizedProducts, null, 2)}\n`, "utf8");
  } catch {
...
export async function readProducts() {
  try {
    ...
    if (!error && Array.isArray(data) && data.length > 0) {
      const mapped = data.map((row) => mapTableRowToCatalogProduct(row));
      return syncProductCatalogCache(mapped);
    }
  } catch {
    // Table or columns missing — fall through to legacy catalog.
  }
  ...
  try {
    const { files } = await getLocalStoreDependencies();
...
/** Rebuild site_settings catalog from the live products table (admin saves). */
export async function syncCatalogFromDatabase() {
  const supabase = getSupabase();
  const { data, error } = await supabase
    .from("products")
    .select("*")
  ...
  if (error) {
    throw new Error(`syncCatalogFromDatabase failed: ${error.message}`);
  }
...
export async function writeProducts(products) {
  const normalizedProducts = normalizeProducts(products);
  lastSyncedProductCatalogSignature = "";
  const supabase = getSupabase();
  ...
  try {
    ...
    // Ignore local file write in serverless.
  }
}
...
export async function queryShopProducts(options = {}) {
