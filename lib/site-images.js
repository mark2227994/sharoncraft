...
const SUPABASE_KEY =
  ...
  "sb_publishable_3CbiLXuCbqRGjKmBp7Ez3w_NV4HaLXa"; // Development/fallback only
...
if (
  ...
  console.warn(
    "[site-images] Warning: Using fallback Supabase key in production. Set SUPABASE_SERVICE_ROLE_KEY or SUPABASE_ANON_KEY for production deployments."
  );
}
...
export async function readSiteImages() {
  const [blobValue, storageValue, tableValue, localValue] = await Promise.all([
    readBlobSiteImages(),
  ...
  const selected = pickNewestContent([blobValue, storageValue, tableValue, localValue]);
  if (selected) {
