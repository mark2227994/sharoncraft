...
export async function writeSiteImages(patch) {
  ...
  try {
    ...
    if (error.message?.toLowerCase().includes("row-level security")) {
      throw new Error(
        "writeSiteImages failed: site content writes need SUPABASE_SERVICE_ROLE_KEY in your env, or a writable local file fallback"
      );
    }
    throw new Error(`writeSiteImages failed: ${error.message}`);
  }
}
