...
function getRequiredFields(product) {
  ...
  if (product?.category === "Jewellery") {
    requiredFields.push("jewelryType");
  }
...
export function getProductCompleteness(product) {
  const requiredFields = getRequiredFields(product);

  if (!product) return { isComplete: false, missingFields: requiredFields, percentage: 0 };
